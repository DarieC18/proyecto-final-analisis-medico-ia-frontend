        ...
        <div class="card-body p-5">
          ...
          <div v-if="tabActual === 'alertas'" class="animation-fade">
            <div class="d-flex justify-content-between align-items-center mb-4">
              ...
              <div class="btn-group btn-group-sm" v-if="alertas.length">
                <button @click="filtroAlertas = 'activas'" class="btn" :class="filtroAlertas === 'activas' ? 'btn-dark' : 'btn-light border'">Activas</button>
                <button @click="filtroAlertas = 'todas'" class="btn" :class="filtroAlertas === 'todas' ? 'btn-dark' : 'btn-light border'">Todas</button>
              </div>
            </div>
            <div v-if="cargandoAlertas" class="text-center py-4">
              <div class="spinner-border text-primary" role="status"></div>
            </div>
            <div v-else-if="alertasFiltradas.length === 0" class="text-muted text-center py-4">
              <p>{{ filtroAlertas === 'activas' ? 'No hay alertas activas.' : 'No hay alertas asociadas a esta cita.' }}</p>
            </div>
            <div v-for="a in alertasFiltradas" :key="a.id" class="card mb-3 border-0 shadow-sm rounded-4" :class="a.isResolved ? 'bg-light' : 'border-start border-4 border-danger'">
              <div class="card-body p-4">
                ...
                <div class="d-flex justify-content-between align-items-center mt-2">
                  ...
                  <button v-if="!a.isResolved" @click="resolverAlerta(a)" class="btn btn-sm btn-success px-3 rounded-pill">
          ...
          <div v-if="tabActual === 'documentos'" class="animation-fade">
            ...
            <div v-if="showDocUpload" class="card bg-light border-0 p-4 rounded-4 mb-4">
              <form @submit.prevent="subirDocumento">
                <div class="row g-3">
                  <div class="col-md-6">
                    <label class="form-label text-muted small fw-bold text-uppercase">Nombre del documento</label>
                    <input v-model="docFileName" type="text" class="form-control bg-white border-0" placeholder="Ej: Resultado examen sangre" required>
                  </div>
                  <div class="col-md-6">
                    <label class="form-label text-muted small fw-bold text-uppercase">Tipo</label>
                    <select v-model="docFileType" class="form-select bg-white border-0">
                      <option>Resultados de laboratorio</option>
                      <option>Indicaciones médicas</option>
                      <option>Historial externo</option>
                      <option>Estudios en PDF</option>
                      <option>Documentos administrativos</option>
                    </select>
                  </div>
                  <div class="col-md-8">
          ...
          <div v-if="tabActual === 'ia'" class="animation-fade">
            ...
            <div v-if="analisisIA.length > 0" class="text-center mb-4">
              <button @click="solicitarAnalisisIA" class="btn btn-outline-dark px-4" :disabled="solicitandoIA">
                <span v-if="solicitandoIA" class="spinner-border spinner-border-sm me-2"></span>
                {{ solicitandoIA ? 'Generando...' : '+ Nuevo Análisis IA' }}
              </button>
            </div>
...
const alertasFiltradas = computed(() => {
  if (filtroAlertas.value === 'activas') return alertas.value.filter(a => !a.isResolved)
...
const solicitarAnalisisIA = async () => {
  ...
  try {
    await aiAnalysisService.generate({ appointmentId: props.id, analysisType: 'Clinical' })
    setTimeout(async () => {
      try {
        const res = await aiAnalysisService.getByAppointment(props.id)
        analisisIA.value = res.data || []
      } catch {
        analisisIA.value = []
      }
    ...
    generandoIA.value = false
    solicitandoIA.value = false
    error.value = 'Error al solicitar análisis IA: ' + (err.response?.data?.message || err.message)
  }
}
...
const actualizarSintoma = async () => {
  guardandoSintoma.value = true
  try {
    const res = await symptomService.update(editSintomaId.value, {
      ...editSintomaForm,
      appointmentId: Number(props.id)
    })
...
const actualizarSigno = async () => {
  guardandoSignos.value = true
  try {
    const res = await vitalSignService.update(editSignoId.value, {
      ...editSignoForm,
      appointmentId: Number(props.id)
    })
...
const guardarRecord = async () => {
  ...
  try {
    const payload = {
      ...
      diagnosisInitial: recordForm.diagnosisInitial || '',
...
const cargarDocumentos = async () => {
  ...
  try {
    const [apptRes, patientRes] = await Promise.all([
      medicalDocumentService.getByAppointment(props.id),
      cita.value?.patientId ? medicalDocumentService.getByPatient(cita.value.patientId) : Promise.resolve({ data: [] })
    ])
    const apptDocs = apptRes.data || []
    const patientDocs = patientRes.data || []
    const merged = [...apptDocs, ...patientDocs.filter(pd => !apptDocs.some(ad => ad.id === pd.id))]
    documentos.value = merged
  } catch {
    documentos.value = []
  } finally {
